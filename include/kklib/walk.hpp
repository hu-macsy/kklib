...
template <typename walker_data_t> struct Walker
{
public:
    walker_id_t id;
    step_t step;
...
template <> struct Walker<EmptyData>
{
public:
    uint32_t id;
    union
...
template <typename edge_data_t> struct AliasBucket
{
    real_t p;
    AdjUnit<edge_data_t>*p_ptr, *q_ptr;
};

template <typename edge_data_t> struct AliasTableContainer
{
...
template <typename query_data_t> struct stateQuery
{
    VertexID src_v;
...
template <> struct stateQuery<EmptyData>
{
    VertexID src_v;
    union
    {
...
template <typename response_data_t> struct stateResponse
{
    walker_id_t walker_idx;
    response_data_t data;
};

template <typename edge_data_t> struct SecondOrderCandidate
{
...
public:
    ...
    bool print_with_head_info{ false };
    bool output_consumer_flag{ false };
    ...
    void set_output_file(const char* output_path_prefix_param, bool with_head_info = false)
    {
        ...
        output_path_prefix = std::string(output_path_prefix_param);
        print_with_head_info = with_head_info;
    }

    void set_output_consumer(std::function<void(PathSet*)> output_consumer_func_param)
    {
        output_consumer_flag = true;
        output_consumer_func = output_consumer_func_param;
    }
...
template <typename edge_data_t, typename walker_data_t> class WalkerConfig
{
public:
    // walker setting
    walker_id_t walker_num{ 0 };
    std::function<VertexID(walker_id_t)> walker_init_dist_func;
    std::function<void(Walker<walker_data_t>&, VertexID)> walker_init_state_func;
    std::function<void(Walker<walker_data_t>&, VertexID, AdjUnit<edge_data_t>*)> walker_update_state_func;

    WalkerConfig(walker_id_t walker_num_param,
                 std::function<void(Walker<walker_data_t>&, VertexID)> walker_init_state_func_param = nullptr,
                 std::function<void(Walker<walker_data_t>&, VertexID, AdjUnit<edge_data_t>*)> walker_update_state_func_param = nullptr,
                 std::function<VertexID(walker_id_t)> walker_init_dist_func_param = nullptr)
    : walker_num(walker_num_param)
    , walker_init_state_func(walker_init_state_func_param)
    , walker_update_state_func(walker_update_state_func_param)
    , walker_init_dist_func(walker_init_dist_func_param)
    {
    }
};

template <typename edge_data_t, typename walker_data_t> class TransitionConfig
{
public:
    // random walk setting
    ...
    std::function<real_t(VertexID, AdjList<edge_data_t>*)> dcomp_lowerbound_func;
    std::function<void(Walker<walker_data_t>&, VertexID, AdjList<edge_data_t>*, real_t&, VertexID&)> outlier_upperbound_func;
    std::function<AdjUnit<edge_data_t>*(Walker<walker_data_t>&, VertexID, AdjList<edge_data_t>*, VertexID)> outlier_search_func;
    ...
    TransitionConfig(std::function<real_t(Walker<walker_data_t>&, VertexID)> extension_comp_func_param,
                     std::function<real_t(VertexID, AdjUnit<edge_data_t>*)> static_comp_func_param = nullptr,
                     std::function<real_t(Walker<walker_data_t>&, VertexID, AdjUnit<edge_data_t>*)> dynamic_comp_func_param = nullptr,
                     std::function<real_t(VertexID, AdjList<edge_data_t>*)> dcomp_upperbound_func_param = nullptr,
                     std::function<real_t(VertexID, AdjList<edge_data_t>*)> dcomp_lowerbound_func_param = nullptr,
                     std::function<void(Walker<walker_data_t>&, VertexID, AdjList<edge_data_t>*, real_t&, VertexID&)> outlier_upperbound_func_param = nullptr,
                     std::function<AdjUnit<edge_data_t>*(Walker<walker_data_t>&, VertexID, AdjList<edge_data_t>*, VertexID)> outlier_search_func_param = nullptr)
    {
        TransitionConfig();
        set_transition(extension_comp_func_param, static_comp_func_param, dynamic_comp_func_param, dcomp_upperbound_func_param,
                       dcomp_lowerbound_func_param, outlier_upperbound_func_param, outlier_search_func_param);
    }

    void set_transition(
        std::function<real_t(Walker<walker_data_t>&, VertexID)> extension_comp_func_param,
        std::function<real_t(VertexID, AdjUnit<edge_data_t>*)> static_comp_func_param = nullptr,
        std::function<real_t(Walker<walker_data_t>&, VertexID, AdjUnit<edge_data_t>*)> dynamic_comp_func_param = nullptr,
        std::function<real_t(VertexID, AdjList<edge_data_t>*)> dcomp_upperbound_func_param = nullptr,
        std::function<real_t(VertexID, AdjList<edge_data_t>*)> dcomp_lowerbound_func_param = nullptr,
        ...
        std::function<AdjUnit<edge_data_t>*(Walker<walker_data_t>&, VertexID, AdjList<edge_data_t>*, VertexID)> outlier_search_func_param = nullptr)
    {
        ...
        assert(outlier_upperbound_func_param == nullptr && outlier_search_func_param == nullptr ||
               outlier_upperbound_func_param != nullptr && outlier_search_func_param != nullptr);
...
public:
    ...
    std::function<real_t(Walker<walker_data_t>&, stateResponse<response_data_t>&, VertexID, AdjUnit<edge_data_t>*)> dynamic_comp_func;
    std::function<real_t(VertexID, AdjList<edge_data_t>*)> dcomp_upperbound_func;
    std::function<real_t(VertexID, AdjList<edge_data_t>*)> dcomp_lowerbound_func;
    std::function<void(Walker<walker_data_t>&, VertexID, AdjList<edge_data_t>*, real_t&, VertexID&)> outlier_upperbound_func;
    std::function<AdjUnit<edge_data_t>*(Walker<walker_data_t>&, VertexID, AdjList<edge_data_t>*, VertexID)> outlier_search_func;
    ...
    SecondOrderTransitionConfig(
        std::function<real_t(Walker<walker_data_t>&, VertexID)> extension_comp_func_param,
        std::function<real_t(VertexID, AdjUnit<edge_data_t>*)> static_comp_func_param,
        std::function<void(Walker<walker_data_t>&, walker_id_t, VertexID, AdjUnit<edge_data_t>*)> post_query_func_param,
        std::function<void(VertexID, stateQuery<query_data_t>&, AdjList<edge_data_t>*)> respond_query_func_param,
        std::function<real_t(Walker<walker_data_t>&, stateResponse<response_data_t>&, VertexID, AdjUnit<edge_data_t>*)> dynamic_comp_func_param,
        std::function<real_t(VertexID, AdjList<edge_data_t>*)> dcomp_upperbound_func_param,
        std::function<real_t(VertexID, AdjList<edge_data_t>*)> dcomp_lowerbound_func_param = nullptr,
        std::function<void(Walker<walker_data_t>&, VertexID, AdjList<edge_data_t>*, real_t&, VertexID&)> outlier_upperbound_func_param = nullptr,
        std::function<AdjUnit<edge_data_t>*(Walker<walker_data_t>&, VertexID, AdjList<edge_data_t>*, VertexID)> outlier_search_func_param = nullptr)
    {
        SecondOrderTransitionConfig();
        set_transition(extension_comp_func_param, static_comp_func_param, post_query_func_param,
                       respond_query_func_param, dynamic_comp_func_param, dcomp_upperbound_func_param,
                       dcomp_lowerbound_func_param, outlier_upperbound_func_param, outlier_search_func_param);
    }
...
template <typename edge_data_t, typename walker_data_t> class WalkEngine : public GraphEngine<edge_data_t>
{
    std::vector<std::unique_ptr<kklib::RandomEngine<VertexID>>> random_engines_int;
    std::vector<std::unique_ptr<kklib::RandomEngine<real_t>>> random_engines_float;
...
public:
    ...
    void set_concurrency(int worker_num_param)
    {
        ...
        for (int w = 0; w < worker_num_param; ++w)
        {
            random_engines_int.emplace_back(std::make_unique<kklib::RandomEngine<VertexID>>(random_seed + omp_get_thread_num()));
            random_engines_float.emplace_back(std::make_unique<kklib::RandomEngine<real_t>>(random_seed + omp_get_thread_num()));
        }
    }

    kklib::RandomEngine<VertexID>& thread_local_random_engine_int()
    {
        return *random_engines_int[omp_get_thread_num()].get();
    }

    kklib::RandomEngine<real_t>& thread_local_random_engine_float()
    {
        return *random_engines_float[omp_get_thread_num()].get();
    }

    std::function<VertexID(walker_id_t)> get_equal_dist_func()
    {
        auto equal_dist_func = [&](walker_id_t w_id)
        {
            VertexID start_v = w_id % this->v_num;
            return start_v;
        };
    ...
    std::function<VertexID(walker_id_t)> get_uniform_dist_func()
    {
        auto uniform_dist_func = [&](walker_id_t w_id)
        {
            // VertexID start_v = get_thread_local_rand_gen()->gen(this->v_num);
            // return start_v;
            return 1;
        };
    ...
    void random_walk(WalkerConfig<edge_data_t, walker_data_t>* walker_config,
                     transition_config_t* transition_config,
                     WalkConfig walk_config = WalkConfig{})
    {
        internal_random_walk_wrap(walker_config, transition_config, walk_config);
    }

private:
    walker_id_t init_walkers(Message<Walker<walker_data_t>>*& local_walkers,
...
#pragma omp parallel for
                for (walker_id_t w_i = walker_begin / this->partition_num * this->partition_num + this->local_partition_id;
                     w_i < walker_end; w_i += this->partition_num)
                {
                    if (w_i < walker_begin)
                    {
                        continue;
                    }
                    VertexID start_v = walker_init_dist_func(w_i);
...
#pragma omp parallel for
    ...
    void init_dcomp_upperbound(std::function<real_t(VertexID, AdjList<edge_data_t>*)> dcomp_upperbound_func, real_t*& dcomp_upperbound)
    {
        assert(dcomp_upperbound == nullptr);
        dcomp_upperbound = this->template alloc_vertex_array<real_t>();

        VertexID local_v_begin = this->get_local_vertex_begin();
        VertexID local_v_end = this->get_local_vertex_end();
#pragma omp parallel for
        for (VertexID v_i = local_v_begin; v_i < local_v_end; v_i++)
        {
    ...
    void init_dcomp_lowerbound(std::function<real_t(VertexID, AdjList<edge_data_t>*)> dcomp_lowerbound_func, real_t*& dcomp_lowerbound)
    {
...
#pragma omp parallel for
    ...
    void init_alias_tables(std::function<real_t(VertexID, AdjUnit<edge_data_t>*)> static_comp_func,
                           ...
                           real_t* regular_area = nullptr)
    {
        ...
        VertexID progress = local_v_begin;
#pragma omp parallel
        {
            int worker_id = omp_get_thread_num();
            const VertexID work_step_length = kklib::parallel_chunk_size;
            VertexID next_workload;
            VertexID* p_set = new VertexID[max_degree];
            VertexID* q_set = new VertexID[max_degree];
            real_t* prob = new real_t[max_degree];
            while ((next_workload = __sync_fetch_and_add(&progress, work_step_length)) < local_v_end)
            {
                VertexID v_begin = next_workload;
                VertexID v_end = std::min(next_workload + work_step_length, local_v_end);
                for (VertexID v_i = v_begin; v_i != v_end; v_i++)
                {
                    ...
                    {
                        real_t sum = 0;
                        for (VertexID e_i = 0; e_i < this->vertex_out_degree[v_i]; e_i++)
                        {
                        ...
                        VertexID p_set_head = 0;
                        VertexID p_set_tail = 0;
                        VertexID q_set_head = 0;
                        VertexID q_set_tail = 0;
                        for (VertexID e_i = 0; e_i < this->vertex_out_degree[v_i]; e_i++)
                        {
                        ...
                        {
                            VertexID p_idx = p_set[p_set_head++];
                            if (q_set_tail != q_set_head)
                            {
                                VertexID q_idx = q_set[q_set_head++];
...
public:
    template <typename query_data_t, typename response_data_t> struct InternalWalkData
    {
        typedef Walker<walker_data_t> walker_t;
        typedef Message<walker_t> walker_msg_t;
    ...
    void internal_random_walk(WalkerConfig<edge_data_t, walker_data_t>* walker_config,
                              transition_config_t* transition_config,
                              WalkConfig const& walk_config,
                              int order)
    {
        typedef Walker<walker_data_t> walker_t;
        typedef Message<walker_t> walker_msg_t;
...
#pragma omp parallel for
        ...
        while (remained_walker != 0)
        {
            ...
            walk_data.local_walker_num = init_walkers(walk_data.local_walkers, walk_data.local_walkers_bak,
                                                      walker_begin, walker_begin + walk_data.active_walker_num,
                                                      walker_config->walker_init_dist_func, walker_config->walker_init_state_func);
...
#pragma omp parallel for
                ...
                {
                    walk_data.pc->add_footprint(Footprint(walk_data.local_walkers[w_i].data.id,
                                                          walk_data.local_walkers[w_i].dst_vertex_id, 0),
                                                omp_get_thread_num());
                }
            }
    ...
    void internal_random_walk_wrap(WalkerConfig<edge_data_t, walker_data_t>* walker_config,
    ...
    template <typename query_data_t, typename response_data_t>
    void internal_walk_epoch(InternalWalkData<query_data_t, response_data_t>* walk_data,
        ...
        while (active_walker_num != 0)
        {
...
#endif
            bool use_parallel = (active_walker_num >= kklib::omp_parallel_threshold);
            active_walker_num = this->template distributed_execute<walker_t>(
                [&](void)
                {
                    ...
                    const walker_id_t work_step_length = kklib::parallel_chunk_size;
#pragma omp parallel if (use_parallel)
                    {
                        int worker_id = omp_get_thread_num();
                        kklib::RandomEngine<real_t>& local_rand_float_engine = thread_local_random_engine_float();
                        kklib::RandomEngine<VertexID>& local_rand_int_engine = thread_local_random_engine_int();
                        ...
                        while ((next_workload = __sync_fetch_and_add(&progress, work_step_length)) < data_amount)
                        {
                            ...
                            for (walker_msg_t* p = begin; p != end; p++)
                            {
                                ...
                                VertexID current_v = p->dst_vertex_id;
                                while (true)
                                {
                                    VertexID const degree = this->vertex_out_degree[current_v];
                                    if (degree == 0)
                                    {
                                        break;
                                    }
                                    ...
                                    {
                                        real_t const extension_comp = extension_comp_func(walker, current_v);
                                        // continue to walk at a probability of extension_comp
                                        if (extension_comp == 0.0 ||
                                            (extension_comp < 1.0 && local_rand_float_engine(0., 1.) > extension_comp))
                                        {
                                            break;
                                        }
                                    }
                                    ...
                                    while (ac_edge == nullptr)
                                    {
                                        if (outlier_opt_flag)
                                        {
                                            ...
                                            VertexID outlier_num_upperbound;
                                            outlier_upperbound_func(p->data, current_v, adj,
                                                                    outlier_overflow_upperbound, outlier_num_upperbound);
                                            if (outlier_overflow_upperbound > 0 && outlier_num_upperbound > 0)
                                            {
                                                float const upper_bound = outlier_overflow_upperbound * outlier_num_upperbound +
                                                                          regular_area[current_v];
                                                real_t randval = local_rand_float_engine(0., upper_bound) - regular_area[current_v];

                                                if (randval > 0)
                                                {
                                                    // fall in appendix area
                                                    VertexID outlier_idx = randval / outlier_overflow_upperbound;
                                                    ...
                                                    {
                                                        real_t outlier_static_comp =
                                                            static_comp_func ? static_comp_func(current_v, candidate) : 1.0;
                                                        randval = (randval - outlier_idx * outlier_overflow_upperbound) / outlier_static_comp +
                                                                  dcomp_upperbound[current_v];
#ifdef UNIT_TEST
                                                        assert(outlier_overflow_upperbound / outlier_static_comp +
                                                                   dcomp_upperbound[current_v] + 1e-6 >=
                                                               dynamic_comp_func(walker, current_v, candidate));
#endif
                                        ...
                                        if (alias_tables == nullptr)
                                        {
                                            candidate = adj->begin + local_rand_int_engine(0, VertexID(degree - 1));
                                        }
                                        else
                                        {
                                            AliasBucket<edge_data_t>* bucket =
                                                alias_tables->index[current_v] + local_rand_int_engine(0, VertexID(degree - 1));
                                            if (bucket->q_ptr == nullptr || local_rand_float_engine(0.f, 1.f) < bucket->p)
                                            {
                                        ...
                                        {
                                            real_t dart_height = local_rand_float_engine(0., dcomp_upperbound[current_v]);
                                            if ((dcomp_lowerbound != nullptr && dart_height <= dcomp_lowerbound[current_v]) ||
                                                dart_height <= dynamic_comp_func(walker, current_v, candidate))
                                            {
                                                ac_edge = candidate;
                                            }
                                        }
                            ...
                            this->notify_progress(begin - data_begin, end - data_begin, data_amount,
                                                  active_walker_num >= kklib::phased_execution_threshold * this->partition_num);
                        }
                    }
                    local_walker_num = 0;
                },
                ...
                local_walkers_bak, active_walker_num >= kklib::phased_execution_threshold * this->partition_num);
        }
    }
...
#pragma omp parallel if (use_parallel)
                            ...
                            for (auto* p = begin; p != end; p++)
                            {
                                ...
                                while (local_cal)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        // continue to walk at a probability of extension_comp
                                        if (extension_comp == 0.0 ||
                                            (extension_comp < 1. && local_rand_float_engine(0., 1.) > extension_comp))
                                        {
                                            terminate_flag = true;
                                        }
                                    }
                                    if (!terminate_flag)
                                    {
                                        ...
                                        {
                                            AdjList<edge_data_t>* adj = this->csr->adj_lists + current_v;
                                            VertexID degree = this->vertex_out_degree[current_v];
                                            bool in_appendix_area = false;
                                            if (outlier_opt_flag)
                                            {
                                                real_t outlier_overflow_upperbound;
                                                VertexID outlier_num_upperbound;
                                                outlier_upperbound_func(p->data, current_v, adj, outlier_overflow_upperbound,
                                                                        outlier_num_upperbound);
                                                if (outlier_overflow_upperbound > 0 && outlier_num_upperbound > 0)
                                                {
                                                    float const upper_bound = outlier_overflow_upperbound * outlier_num_upperbound +
                                                                              regular_area[current_v];
                                                    real_t randval =
                                                        local_rand_float_engine(0., upper_bound) - regular_area[current_v];
                                                    if (randval > 0)
                                                    {
                                                        ...
                                                        {
                                                            real_t outlier_static_comp =
                                                                static_comp_func ? static_comp_func(current_v, cd->candidate) : 1.0;
                                                            cd->randval = (randval - outlier_idx * outlier_overflow_upperbound) /
                                                                              outlier_static_comp +
                                                                          dcomp_upperbound[current_v];
                                            ...
                                            if (!in_appendix_area)
                                            {
                                                if (alias_tables == nullptr)
                                                {
                                                    cd->candidate = adj->begin + local_rand_int_engine(0, VertexID(degree - 1u));
                                                }
                                                else
                                                {
                                                    AliasBucket<edge_data_t>* bucket =
                                                        alias_tables->index[current_v] +
                                                        local_rand_int_engine(0, VertexID(degree - 1u));
                                                    if (bucket->q_ptr == nullptr ||
                                                        local_rand_float_engine(0., 1.) < bucket->p)
                                                    {
                                                ...
                                                cd->randval = local_rand_float_engine(0., dcomp_upperbound[current_v]);
                                            ...
                                            if (cd->accepted == true)
                                            {
                                                if (this->is_local_vertex(cd->candidate->neighbour))
                                                {
                                                    ...
                                                    if (output_flag)
                                                    {
                                                        pc->add_footprint(Footprint(p->data.id, cd->candidate->neighbour,
                                                                                    p->data.step),
                                                                          worker_id);
                                                    }
#ifdef COLLECT_WALK_SEQUENCE
                                                    footprints[worker_id].push_back(
                                                        Footprint(p->data.id, cd->candidate->neighbour, p->data.step));
#endif
                                                }
                                                else
                                                {
                ...
                [&](Message<query_t>* msg_begin, Message<query_t>* msg_end) { cached_request_end = msg_end; },
                cached_request, active_walker_num >= kklib::phased_execution_threshold * this->partition_num);
...
#pragma omp parallel if (use_parallel)
                ...
                nullptr, active_walker_num >= kklib::phased_execution_threshold * this->partition_num);
...
#pragma omp parallel if (use_parallel)
                    {
                        ...
                        while ((next_workload = __sync_fetch_and_add(&progress, work_step_length)) < data_amount)
                        {
                            ...
                            for (walker_msg_t* p = begin; p != end; p++)
                            {
                                ...
                                if (cd->candidate != nullptr)
                                {
                                    if (cd->accepted || cd->randval <= dynamic_comp_func(walker, remote_response_cache[walker_idx],
                                                                                         current_v, cd->candidate))
                                    {
                                        walker.step++;
...
#ifdef COLLECT_WALK_SEQUENCE
                                        footprints[worker_id].push_back(
                                            Footprint(walker.id, cd->candidate->neighbour, walker.step));
#endif
                                    }
                                    else
                                    {
...
#ifdef COLLECT_WALK_SEQUENCE
    void collect_walk_sequence(std::vector<std::vector<VertexID>>& sequence, walker_id_t walker_num)
    {
        std::thread send_thread(
            [&]()
            {
                ...
                {
                    MPI_Send(footprints[t_i].data(), footprints[t_i].size() * sizeof(Footprint), MPI_CHAR, 0, Tag_Msg, MPI_COMM_WORLD);
        ...
        if (get_mpi_rank() == 0)
        {
            ...
            for (partition_id_t p_i = 0; p_i < get_mpi_size(); p_i++)
            {
                for (int t_i = 0; t_i < this->worker_num; t_i++)
                {
                    ...
                    MPI_Recv(recv_data.data(), recv_size, MPI_CHAR, p_i, Tag_Msg, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    void collect_walker_init_state(std::vector<Walker<walker_data_t>>& output)
    {
        if (get_mpi_rank() != 0)
        {
            MPI_Send(walker_init_state.data(), walker_init_state.size() * sizeof(Walker<walker_data_t>), MPI_CHAR, 0,
                     Tag_Msg, MPI_COMM_WORLD);
        }
        else
        {
            ...
            std::sort(output.begin(), output.end(),
                      [](const Walker<walker_data_t> a, const Walker<walker_data_t> b) { return a.id < b.id; });
        }
    }
#endif
};
